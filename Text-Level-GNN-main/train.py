...
class TextLevelGNN(nn.Module):
    def __init__(self, pretrained_embeddings, out_size=2, dropout_rate=0, padding_idx=1):
...
dataset = TextLevelGNNDatasetClass(train_filename='chess-train-cleaned.txt',
                                   test_filename='chess-test-cleaned.txt',
...
for epoch in range(args.epoch):
    ...
    if epoch >= warm_up_epoch:
        if epoch == warm_up_epoch: # after warm up period, start tracking best_epoch
        ...
        if early_stopping_criteria == 'accuracy':
            if validation['accuracy'][epoch] > validation['accuracy'][best_epoch]:
                ...
                print(f'Early stopping... (No further increase in validation accuracy) for {early_stopping_patience} consecutive epochs.')
                break
        if early_stopping_criteria == 'loss':
            if validation['loss'][epoch] < validation['loss'][best_epoch]:
                best_epoch = epoch
            elif epoch >= best_epoch + early_stopping_patience:
                print(f'Early stopping... (No further decrease in validation loss) for {early_stopping_patience} consecutive epochs.')
                break
...
df.to_csv(f'results/es={args.embedding_size},p={args.p},mf={args.min_freq},ml={args.max_length},d={args.dropout},l={args.lr},ldf={args.lr_decay_factor},lde={args.lr_decay_every},wd={args.weight_decay},wue={args.warm_up_epoch},esp={args.early_stopping_patience},esc={args.early_stopping_criteria},e={args.epoch}.csv') # Logging
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.savefig((f'figures/loss/es={args.embedding_size},p={args.p},mf={args.min_freq},ml={args.max_length},d={args.dropout},l={args.lr},ldf={args.lr_decay_factor},lde={args.lr_decay_every},wd={args.weight_decay},wue={args.warm_up_epoch},esp={args.early_stopping_patience},esc={args.early_stopping_criteria},e={args.epoch}.png'))
plt.show()
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.savefig((f'figures/acc/es={args.embedding_size},p={args.p},mf={args.min_freq},ml={args.max_length},d={args.dropout},l={args.lr},ldf={args.lr_decay_factor},lde={args.lr_decay_every},wd={args.weight_decay},wue={args.warm_up_epoch},esp={args.early_stopping_patience},esc={args.early_stopping_criteria},e={args.epoch}.png'))
plt.show()